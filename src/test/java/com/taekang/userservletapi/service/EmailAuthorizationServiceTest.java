...
class EmailAuthorizationServiceTest {
  ...
  @Autowired private EmailAuthorizationService emailAuthorizationService;
  ...
  //        doThrow(new MailSendException("메일 전송
  // 실패")).when(javaMailSender).send(any(MimeMessage.class));
  //
  //        assertThrows(FailedMessageSendException.class, () ->
  // emailAuthorizationService.sendAuthMail(email));
  //    }
}
