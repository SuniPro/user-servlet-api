...
public class MailSenderService {
  ...
  public MimeMessage createDepositApprovalMail(String mail, CryptoDepositDTO cryptoDeposit) {
    MimeMessage mimeMessage = javaMailSender.createMimeMessage();
    ...
    try {
      ...
      String body =
          "<!DOCTYPE html><html><head>  <meta charset=\"UTF-8\">  <title>이메일 인증</title></head><body"
              ...
              + " #ffffff; padding: 30px; border-radius: 10px; box-shadow: 0 2px 5px"
              + " rgba(0,0,0,0.1);\">    <h2 style=\"color: #333333;\"><span style=\"color:"
              + " #037AED;\">i coins</span> 입금 승인 안내</h2>    <p style=\"font-size: 16px; color:"
              + " #555555;\">안녕하세요. 요청하신"
              + cryptoDeposit.getAmount()
              + cryptoDeposit.getCryptoType()
              + " 입금 건이 승인되었습니다.<br><br>  이용해주셔서 감사합니다.</p>  </div>"
              + "</body></html>";
      mimeMessage.setContent(body, "text/html; charset=UTF-8");
