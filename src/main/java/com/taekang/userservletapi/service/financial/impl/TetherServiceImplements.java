...
public class TetherServiceImplements implements TetherService {
    ...
    public TetherAccountAndDepositDTO createOrFindTetherAccount(TetherCreateDTO dto) {
        boolean existAccount = tetherAccountRepository.existsByTetherWalletAndUsername(dto.getTetherWallet(), dto.getUsername());
        ...
        if (existAccount) {
            account = tetherAccountRepository.findByTetherWallet(dto.getTetherWallet())
                    .orElseThrow(AccountNotFoundException::new);
            latestDepositOpt = tetherDepositRepository.findTopByTetherAccount_TetherWalletOrderByRequestedAtDesc(account.getTetherWallet());
            return toDto(account, latestDepositOpt);
        } else if (existsUsername || existsWallet) {
            throw new WalletVerification(); // 중복 but 일치하지 않음
        } else {
            // 신규 등록
            account = tetherAccountRepository.save(
    ...
    private TetherAccountAndDepositDTO toDto(TetherAccount account, Optional<TetherDeposit> latestDepositOpt) {
