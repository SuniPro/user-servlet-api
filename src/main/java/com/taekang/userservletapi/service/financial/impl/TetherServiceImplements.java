...
public class TetherServiceImplements implements TetherService {
    ...
    public TetherAccount createOrFindTetherAccount(TetherAccountDTO dto) {
        return tetherAccountRepository.findByTetherWallet(dto.getTetherWallet())
                .orElseGet(() -> {
                    TetherAccount account = TetherAccount.builder()
    ...
    public TetherDeposit createDeposit(TetherDepositRequestDTO dto) {
        TetherAccount tetherAccount = tetherAccountRepository.findByTetherWallet(dto.getTetherWallet()).orElseThrow(AccountNotFoundException::new);

        if (dto.getAmount().compareTo(BigDecimal.ZERO) <= 0) {
            throw new InvalidAmountException();
        }

        TetherDeposit tetherDeposit = TetherDeposit.builder()
